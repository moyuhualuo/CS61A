def duplicate_link(link, val):
    ...
    while current:
        if current.first == val:
            new_code = Link(val, current.rest)
            current.rest = new_code
...
def multiply_lnks(lst_of_lnks):
    ...
    for link in lst_of_lnks:
        if link is Link.empty:
            ...
            product *= link.first
    lst_of_lnks_rests = [link.rest for link in lst_of_lnks if link is not Link.empty]
