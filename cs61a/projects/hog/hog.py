...
def play(strategy0, strategy1, update, score0=0, score1=0, dice=six_sided, goal=GOAL):
    ...
    # BEGIN PROBLEM 5
    # 0 is player0 and 1 is player 1
...
def always_roll(n):
    ...
    # BEGIN PROBLEM 6
    def now_score(score, opponent_score):
        return n
...
def make_averaged(original_function, samples_count=1000):
    ...
    def make_AVGinner(*args):
        times = samples_count
        tol = 0
        while times:
            tol += original_function(*args)
            times -= 1
        return tol / samples_count

    return make_AVGinner
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    i, result, Avg = 1, 0, 0
    while i <= 10:
        pre_Avg = make_averaged(roll_dice, samples_count)(i, dice)
        if pre_Avg > Avg:
            result = i
            Avg = pre_Avg
        i += 1
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    if sus_points(boar_brawl(score, opponent_score) + score) >= threshold + score:
        return 0
    else:
        return num_rolls  # Remove this line once implemented.
    # END PROBLEM 11
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    scoring = sus_update(0, score, opponent_score)
    if scoring - score >= make_averaged(always_roll(6))(score, opponent_score):
        return 0
    else:
